	...
	combo_alvo_options.each(function(){
		...
		if(option_val!=''){	
			...
			campos_gatilhos_valores.forEach(function(campo_gatilho){			
				...
				// log('CAMPO GATILHO => nome='+campo_gatilho_nome+' |
				// valor='+campo_gatilho_valor+'');
				
				// ------------------------------
				var option_campo_valor = option.attr(campo_gatilho_nome);			
				// log('SELECT'+combo_selector+'
				// OPTION['+campo_gatilho_nome+'='+option_campo_valor+']');
...
function combo_filtrar(combo_alvo_selector,campos=[]){
	...
	{// levantamento dos campos - percorrimento das opcoes do combo alvo
		// para averigracao dos atributos
